...
{
    int i;
    write(open_fd, "client amount:", sizeof("client amount:"));
    char string[16] = {0};
    sprintf(string, "%d", conn_amount);
    write(open_fd, string, sizeof(string));
    write(open_fd, "\n", sizeof("\n"));
}
void *server_listen(void *arg)
{
    ...
    write(open_fd,"server_listen thread work!\n",sizeof("server_listen thread work!\n"));
    ...
    while (1)
    {
            ...
            if (FD_ISSET(fd_A[i], &fdsr))
            {
                ...
                { // receive data
                    if (ret < BUF_SIZE)
                        memset(&buf[ret], '\0', 1);
                    char string[300] = {0};
                    sprintf(string, "client[%d] send:%s\n", i, buf);
                        write(open_fd, string, sizeof(string));
                    //回复
                    strncpy(title, buf, 8);
        ...
        if (FD_ISSET(sock_fd, &fdsr))
        {
            ...
            if (conn_amount < BACKLOG)
            {
                ...
                char string[300] = {0};
                sprintf(string,"new connection client[%d] %s:%d\n", conn_amount,
...
int main(void)
{
    ...
    if (open_fd != -1)
        close(open_fd);
    open_fd = open("/tmp/server.log", O_RDWR | O_CREAT | O_TRUNC, 0644);
    ...
    //提示服务器端启动
    char string[100] = {0};
    sprintf(string, "%s", ctime(&timep));
    write(open_fd, string, sizeof(string));
    write(open_fd, "   Server work!\n", sizeof("   Server work!\n"));
